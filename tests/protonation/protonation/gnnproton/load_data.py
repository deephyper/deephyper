...
def load_data_sparse(edge=False, full=False, **kwargs):
    # define directories
    ...
    with open(DATA_DIR + "protonation_data_sparse.pickle", "rb") as handle:
        [A_train, X_train, E_train, M_train, N_train, y_train] = pickle.load(handle)
        [A_valid, X_valid, E_valid, M_valid, N_valid, y_valid] = pickle.load(handle)
    ...
    if full is False:
        if edge is False:
            return ([X_train[::2, ...], A_train[::2, ...], E_train[::2, ...], M_train[::2, ...], N_train[::2, ...]], y_train[::2, ...]), \
