...
def extension_from_parameters_old(args):
...
def run(params):
    ...
    print(args.__dict__)
    #ext = extension_from_parameters(args.__dict__)
    ...
    while fold < cv:
        ...
        if args.gen:
            ...
            log_evaluation(evaluate_prediction(y_val, y_shuf), description='Between random pairs in y_val:')
            history = model.fit(x_train_list, y_train, batch_size=args.batch_size, shuffle=args.shuffle, 
                                initial_epoch=initial_epoch, epochs=args.epochs,
