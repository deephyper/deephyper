...
def dump(res, filename, store_objective=True, **kwargs):
    ...
    store_objective : boolean, default=True
        ...
        critical, one can delete it before calling `deephyper.hpo.skopt.dump()` and thus
...
def load(filename, **kwargs):
    ...
    persisted with deephyper.hpo.skopt.dump.

    .. note::
        Notice that the loaded optimization result can be missing
        the objective function (`.specs['args']['func']`) if `deephyper.hpo.skopt.dump`
...
def cook_initial_point_generator(generator, **kwargs):
    ...
    generator : "lhs", "sobol", "halton", "hammersly", "grid", "random" \
        ...
        Should inherit from `deephyper.hpo.skopt.sampler.InitialPointGenerator`.
...
def dimensions_aslist(search_space):
    ...
    search_space : dict
        ...
        :class:`deephyper.hpo.skopt.space.Dimension` (Real, Integer or Categorical)

    Returns
    -------
    params_space_list: list
        list of deephyper.hpo.skopt.space.Dimension instances.

    Examples
    --------
    >>> from deephyper.hpo.skopt.space.space import Real, Integer
...
def point_asdict(search_space, point_as_list):
    ...
    .. seealso:: :class:`deephyper.hpo.skopt.utils.point_aslist`
...
def point_aslist(search_space, point_as_dict):
    ...
    .. seealso:: :class:`deephyper.hpo.skopt.utils.point_asdict`
