...
from deephyper.hpo.skopt.utils import dump
...
class CheckpointSaver(object):
    """
    Save current state after each iteration with :class:`deephyper.hpo.skopt.dump`.
    ...
    >>> def obj_fun(x):
    ...     return x[0]**2
    >>> checkpoint_callback = deephyper.hpo.skopt.callbacks.CheckpointSaver("./result.pkl")
    >>> deephyper.hpo.skopt.gp_minimize(obj_fun, [(-2, 2)], n_calls=10,
    ...
    dump_options : string
        options to pass on to `deephyper.hpo.skopt.dump`, like `compress=9`
    """
