...
def load_data(zero_padding=True, split='random'):
    """
    Load qm8 dataset from .mat file. Max atom 23.
    ...
    Returns:
        Lists of train, valid and test data, and 12 qm8 tasks.
    """
    print("Loading QM8 Dataset (takes about 1 minute)...")
    qm8_tasks, (train_dataset, valid_dataset, test_dataset), transformers = load_qm8(featurizer='GraphConv',
                                                                                     split=split)
    MAX_ATOM = 9
    N_FEAT = 75
