...
def add_subparser(subparsers):
    ...
    subparser.add_argument('-p', '--problem', required=True, help='Problem definition path or object')
    subparser.add_argument('-r', '--run', required=True, help='Run function path or object')

    subparser.add_argument('-t', '--time-minutes', type=int, required=True, help='Job walltime (minutes)')
    subparser.add_argument('-n', '--nodes', type=int, required=True, help='Number of nodes')
    ...
    subparser.add_argument('-j', '--job-mode', required=True, help='Launcher job mode', choices=['mpi', 'serial'])
    subparser.add_argument('--nas-nodes', default=1, type=int, help='Number of nodes over which to parallelize NAS')
