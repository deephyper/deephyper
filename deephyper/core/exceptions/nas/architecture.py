...
class WrongSequenceToSetOperations(NASError):
    """Raised when a sequence of actions is not of the same lenght as the number of variable nodes of the architecture."""
...
class StructureHasACycle(NASError):
    """Raised when a architecture is containing a cycle."""
...
class InputShapeOfWrongType(NASError):
    """Raised when an input shape of a architecture is of a wrong type.
    """
...
class NodeAlreadyAdded(NASError):
    """Raised when a node has already been added in a architecture.
    """
    ...
    def __str__(self):
        return f"The node '{str(self.node)}' has already been added to the architecture."


class WrongOutputShape(NASError):
    """Raised when the output shape of the model generated by a architecture doesn't
