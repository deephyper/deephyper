...
class Cell:
    ...
    def __init__(self, inputs=None):
        ...
        self.labels = {} if inputs is None else {n:str(n) for n in inputs}
    ...
    def set_outputs(self, kind='stack', **kwargs):
        '''
        ...
        if kind == 'stack':
            ...
            if kwargs.get('axis'):
                op = Merge(self.graph, output_node, stacked_nodes, axis=kwargs['axis'])
            else:
                op = Merge(self.graph, output_node, stacked_nodes)
        elif kind == 'concat':
            stacked_nodes = []
            for b in self.blocks:
                stacked_nodes.extend(b.outputs)
            if kwargs.get('axis'):
                op = Concat(self.graph, output_node, stacked_nodes, axis=kwargs['axis'])
            else:
                op = Concat(self.graph, output_node, stacked_nodes)

        output_node.add_op(op)
        output_node.set_op(0)
        self.labels[output_node] = str(output_node)
    ...
    def add_block(self, block):
        ...
        # self.graph.add_nodes_from(block.graph.nodes())
