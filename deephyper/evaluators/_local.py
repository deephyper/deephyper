...
class LocalEvaluator(evaluate.Evaluator):
    """Evaluator to run locally.

        The ``LocalEvaluator`` uses the ``subprocess`` package. The generated processes have a fresh memory independant from their parent process. All the imports are going to be repeated.
