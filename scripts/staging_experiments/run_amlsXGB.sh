...
aprun -n $COBALT_JOBSIZE -N 1 python -c "from mpi4py import MPI; comm=MPI.COMM_WORLD; print('mpi4py test: hello from rank', comm.Get_rank())"


# STAGING PYTHON: COPY TO LOCAL
# --------------------------------
echo "Copying Python environment to $STAGE_IN_DIR"
time (aprun -n $COBALT_JOBSIZE -N 1 -cc None ./stage_python.sh $STAGE_IN_DIR $DEEPHYPER_ENV_PATH $DEEPHYPER_TOP)

DEEPHYPER_ENV_PATH=$STAGE_IN_DIR/$(basename $DEEPHYPER_ENV_PATH)
DEEPHYPER_TOP=$STAGE_IN_DIR/$(basename $DEEPHYPER_TOP)
PYTHON_EXE=$DEEPHYPER_ENV_PATH/bin/python

# Job naming
BNAME=$1
METHOD="amls_XGB_QR_pythonstaged"
...
balsam app --name search --desc 'run AMLS: XGB + QR' --executable "$PYTHON_EXE -u $SEARCH_APP_PATH" --no-check-path
...
# -------------------------
aprun -n $COBALT_JOBSIZE -N 1 -cc none python $BALSAM_PATH/launcher/mpi_ensemble_pull.py --time-limit-min=$(( $WALLMINUTES ))
