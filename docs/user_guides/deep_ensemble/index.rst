...
.. literalinclude:: search_space.py
...
.. literalinclude:: problem_agebo.py
...
Now that all of the components, that is the functions loading the data, the search space and the problem, we can run a search to generate all the models we need.
...
Probabilistic models (UQ)
=========================

Python API is located at ``deephyper.ensemble``.

.. code-block:: python

    ensemble = UQBaggingEnsemble(
        model_dir,
        loss,
        batch_size=None,
        size=5,
...
Example with ``UQBagging``

::

    deephyper ensemble uqbagging --problem nas_ensemble.mnist.problem_agebo.Problem --model-dir save/model/ --size 2 -v 1 --batch-size 1 --mode classification --selection greedy
