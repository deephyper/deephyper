...
def is_failure(value) -> bool:
    ...
    return value == OBJECTIVE_VALUE_FAILURE or (
        isinstance(value, list) and OBJECTIVE_VALUE_FAILURE in value
    )
...
class Optimizer(object):
    ...
    def __init__(
        ...
        if acq_optimizer not in [
            ...
            raise ValueError(
                "Expected acq_optimizer to be 'lbfgs' or 'sampling' or 'ga', got {0}".format(
                    acq_optimizer
                )
            )
