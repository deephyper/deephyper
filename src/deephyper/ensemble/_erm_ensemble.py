...
from deephyper.ensemble.selector._erm_greedy import ERMGreedySelector
...
class ERMEnsemblePredictor(Predictor):
    ...
    def __init__(
        ...
        self.selector = ERMGreedySelector(
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        self.selector.fit(X, y, y_predictors)
    ...
    def predict(self, X: np.ndarray):
        ...
        """
        y_predictors = self.predictions_from_predictors(X, self.predictors)
        y_dummy = self.aggregator.aggregate(y_predictors[:1])
        if isinstance(y_dummy, dict):
            for k in y_dummy:
                y_dummy[k] = y_dummy[k][:1]
            y = {k: [] for k in y_dummy}
        else:
            y_dummy = y_dummy[:1]
            y = []
        ...
        for i in tqdm(range(len(X))):
            ...
            if isinstance(y_predictors[0], dict):
                y_predictors_i = [{k: y_[k][i : i + 1] for k in y_} for y_ in y_predictors]
            else:
                y_predictors_i = [y_[i : i + 1] for y_ in y_predictors]
            indexes, weights = self.selector.select(X_i, y_predictors_i)

            y_predictors_i = [y_predictors_i[j] for j in indexes]

            y_i = self.aggregator.aggregate(y_predictors_i, weights=weights)
